...
def build_parser():
    ...
    parser.add_argument(
        "-c", "--command", type=str, help="Command to run on the container", default=""
    )
    ...
    parser.add_argument(
        ...
        "--ports",
        type=str,
        help="List of comma separated ports to bind",
        default="",
    )
    parser.add_argument(
        "-m",
        "--metrics",
        type=str,
        help="Container port where to listen to metrics",
        default="",
    )
    ...
    try:
        ...
        if not name:
            client.containers.run(
                image=image, command=command, detach=True, ports=list_port
            )
        else:
            client.containers.run(
                image=image, command=command, detach=True, name=name, ports=list_port
            )
...
if __name__ == "__main__":
    with collect_agent.use_configuration(
        "/opt/openbach/agent/jobs/container_run/container_run_rstat_filter.conf"
    ):
